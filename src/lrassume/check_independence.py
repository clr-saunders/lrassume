...
def check_independence(df: pd.DataFrame, target: str) -> dict:
    ...
    if len(X_cols) == 0:
        raise ValueError("No numeric feature columns found in DataFrame (excluding target).")
    
    X = df[X_cols].values
    
    # Check for missing values
    if np.isnan(X).any() or np.isnan(y).any():
        raise ValueError("DataFrame contains missing values. Please handle missing values before using this function.")
